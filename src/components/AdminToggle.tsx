...
export function AdminToggle() {
  const { role, toggleAdmin } = useAuth();
  const active = role === "admin";
  return (
    <button
      onClick={toggleAdmin}
      className={`group inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-semibold uppercase tracking-widest transition-colors ${
        active
