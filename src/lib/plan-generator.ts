...
export function generatePlan(intake: Intake): GeneratedPlan {
  ...
  const bmr = Math.round(
    10 * intake.weightKg + 6.25 * intake.heightCm - 5 * intake.age + s,
  );
  ...
  const trainingSplit =
    splitByDays[Math.max(2, Math.min(6, intake.daysPerWeek))] ?? "Full-Body × 3";
  ...
  const notes: string[] = [];
  if (intake.restrictions.length)
    notes.push(`Strict zero: ${intake.restrictions.join(", ")}.`);
...
export const upsellCatalog = [
  ...
  {
    id: "premium-meal-pack",
    name: "Premium Meal Pack",
    tagline: "8-week rotation + grocery list (West-African biased)",
    amount: 15_000_00,
    perks: ["56 meals", "Auto grocery list", "Onion-free variants"],
    badge: "Most Picked",
  },
  {
    id: "accountability-monthly",
    name: "Accountability Loop",
    tagline: "Weekly WhatsApp check-ins + form review",
    amount: 10_000_00,
    perks: ["Weekly video review", "Photo audit", "Slack-style replies <12h"],
    badge: "Subscription",
  },
] as const;
