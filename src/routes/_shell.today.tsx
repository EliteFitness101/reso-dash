...
function TodayPage() {
      ...
      <section className="space-y-4">
        <div className="glass-card-gold rounded-3xl p-6">
          ...
          <p className="mt-3 text-sm leading-6 text-muted-foreground">
            Your companion learns what matters to you, then turns it into one clear next step across movement, food, recovery and progress.
          </p>
          <Link
            to="/onboarding"
            className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl gold-bg py-3.5 text-xs font-bold uppercase tracking-[0.16em] text-background"
          >
            Personalize My Journey <ArrowRight size={15} />
          </Link>
        </div>
      </section>
    );
  ...
  const completeFirstPlan = () => awardXP("first-plan");
    ...
    <section className="space-y-4">
      ...
      <div className="glass-card-gold rounded-3xl p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          You’re a {experience} working toward {goal}. Start with a practical session using {equipment}. Consistency beats intensity today.
        </p>
        <button
          type="button"
          onClick={completeFirstPlan}
          className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl gold-bg py-3.5 text-xs font-bold uppercase tracking-[0.16em] text-background"
        >
          Mark Today’s Action Complete <CheckCircle2 size={15} />
        </button>
      </div>
      ...
      <div className="glass-card rounded-3xl p-5">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.25em] text-gold">Your journey</p>
            <h2 className="mt-1 font-display text-lg font-semibold">Current state → next state</h2>
          </div>
          <Link to="/checkin" className="text-xs text-gold">Open →</Link>
        </div>
        <div className="mt-4 grid gap-2 text-xs">
          <div className="rounded-2xl border border-border bg-background/30 p-3">
            <p className="text-[9px] uppercase tracking-widest text-muted-foreground">Current state</p>
            <p className="mt-1 font-medium">Building a repeatable wellness routine</p>
          </div>
          <div className="rounded-2xl border border-gold/20 bg-gold/5 p-3">
            <p className="text-[9px] uppercase tracking-widest text-gold">Next best state</p>
            <p className="mt-1 font-medium">Complete your first 7-day consistency cycle</p>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card rounded-3xl p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-[10px] uppercase tracking-[0.25em] text-gold">Milestone engine</p>
            <h2 className="mt-1 font-display text-lg font-semibold">Earn XP by doing, not browsing.</h2>
          </div>
          <span className="font-mono text-[10px] text-muted-foreground">{XP_MILESTONES.length} milestones</span>
        </div>
        <p className="mt-2 text-xs leading-5 text-muted-foreground">
          Your biggest Tier 1 milestone is sharing a genuine progress report. The share flow includes @resoflex and the approved ResoFit tags.
        </p>
      </div>
    </section>
  );
...
function ActionCard({
  ...
  return (
    <Link to={href} className="glass-card rounded-2xl p-4 transition-transform active:scale-[0.98]">
